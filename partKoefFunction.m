function [u,v,u1,v1, izl] = partKoefFunction(z0,kLeft,  kRight)
U = 1;
l = 0.5;
...
for i = 1:length(t)
    ...
    if t(1,i) <= z0
        ...
        u(1,i) = C(3,1)*cos(x)*sinh(x) + C(4,1)*sin(x)*cosh(x) - U*cos(x)*cosh(x) + U;
        ...
        u(1,i) = (C(1,1)*sin(x) + C(2,1)*cos(x))*exp(-x) + U;
